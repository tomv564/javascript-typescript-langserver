...
import { NoPreloader, Preloader } from './preloader'
...
export class ProjectManager implements Disposable {
    ...
    constructor(
        ...
        this.preloader = new NoPreloader(inMemoryFileSystem, updater, this.logger)
        // new RealPreloader(this.rootPath, this.inMemoryFs, (uri) => this.getConfiguration(uri).getHost(), this.updater)
        ...
        this.subscriptions.add(
            Observable.fromEvent(inMemoryFileSystem, 'add', Array.of as SelectorMethodSignature<[string, string]>)
                ...
                .subscribe(([uri, content]) => {
                    ...
                    this.logger.warn(`got [jt]sconfig.json at ${filePath} from filesystem, replacing ${configType}}`)
    ...
    public ensureModuleStructure(childOf = new Span()): Observable<never> {
        return this.preloader.ensureModuleStructure(childOf)
    }
    ...
    public ensureOwnFiles(childOf = new Span()): Observable<never> {
        return this.preloader.ensureOwnFiles(childOf)
     ...
     * Invalidates project configurations after execution
     */
    public ensureAllFiles(childOf = new Span()): void {
        return this.preloader.ensureAllFiles(childOf)
    ...
    public ensureReferencedFiles(uri: string, maxDepth = 30, ignore = new Set<string>(), childOf = new Span()): Observable<string> {
        return this.preloader.ensureReferencedFiles(uri, maxDepth, ignore, childOf)
    ...
    public ensureConfigDependencies(childOf = new Span()): Observable<never> {
        return this.preloader.ensureConfigDependencies()
    }
